...
public class RegistrarRechazoHandlerTests
{
    private static (
        Mock<IRegistroTrabajoRepository> registroRepo,
        Mock<IOperacionRepository> operacionRepo,
        Mock<ITipoRechazoRepository> tipoRechazoRepo,
        Mock<IUnitOfWork> uow) BuildMocks()
    {
        ...
        return (registroRepo, new Mock<IOperacionRepository>(), new Mock<ITipoRechazoRepository>(), uow);
    }

    [Fact]
    public async Task Handle_RegistroNoEncontrado_Throws()
    {
        var (registroRepo, operacionRepo, tipoRechazoRepo, _) = BuildMocks();
        registroRepo.Setup(r => r.GetRegistroActivoPorOperacionAsync(It.IsAny<Guid>()))
            .ReturnsAsync((RegistroTrabajo?)null);

        var handler = new RegistrarRechazoHandler(registroRepo.Object, operacionRepo.Object, tipoRechazoRepo.Object);

        await Assert.ThrowsAsync<Exception>(() =>
            handler.Handle(new RegistrarRechazoCommand { idOperacion = Guid.NewGuid(), Cantidad = 1, IdTipoRechazo = 1 }, CancellationToken.None));
    }
    ...
    public async Task Handle_CantidadSuperaCapacidad_Throws()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            handler.Handle(new RegistrarRechazoCommand { idOperacion = operacionId, Cantidad = 3, IdTipoRechazo = 1 }, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_Exito_GuardaYDevuelveDTO()
    {
        var (registroRepo, operacionRepo, tipoRechazoRepo, uow) = BuildMocks();
        var operacionId = Guid.NewGuid();
        ...
        tipoRechazoRepo.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(new TipoRechazo("Defecto"));

        var handler = new RegistrarRechazoHandler(registroRepo.Object, operacionRepo.Object, tipoRechazoRepo.Object);
        var result = await handler.Handle(
            new RegistrarRechazoCommand { idOperacion = operacionId, Cantidad = 2, IdTipoRechazo = 1, Comentario = "prueba" },
            CancellationToken.None);

        Assert.Equal(2, result.Cantidad);
        Assert.Equal(Cantidad.Create(2), registro.TotalRechazado);
