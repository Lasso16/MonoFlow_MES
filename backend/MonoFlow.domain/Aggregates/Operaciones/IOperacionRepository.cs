...
namespace MonoFlow.domain.Aggregates.Operaciones
{
    public interface IOperacionRepository : IRepository<Operacion>
    {
        ...
        Task<Operacion?> GetByIdAsync(Guid id);
        Task<Operacion?> GetByIdWithRegistrosAsync(Guid id);
        Task<List<Operacion>> GetByArticuloIdAsync(Guid articuloId);
        Task<(List<Operacion> Items, int Total)> GetAllPagedAsync(
